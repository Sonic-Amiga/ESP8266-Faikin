...
static struct S21State init_state = {
   ...
   .protocol_major = 3,  // Protocol version, maximum number we know
   .protocol_minor = 40,
   ...
   // FU15...FU45 modified by hands to correlate with get_monitordata fields and yueld unique recognizable value for each
   .FU15     = {0x31, 0x20, 0x20, 0x20, 0x44, 0x20, 0x20, 0x20, 0x20, 0x20, 0x3A, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
                0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF},
   .FU25     = {0x32, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0xFF,
                0XFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF},
   .FU35     = {0x33, 0x30, 0x51, 0x52, 0x32, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x53, 0x30, 0x30, 0x30,
                0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF},
   .FU45     = {0x34, 0x20, 0x41, 0x42, 0x35, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x43, 0x20, 0x20, 0x20,
   ...
   while (1)
   {
	  ...
	  if (state->protocol_major > 2 && len >= S21_MIN_V3_PKT_LEN &&
		 ...
		 case '0':
			...
			break;
		 case '1':
		    // FY15 - production info. Reported by BRP072C42 in /aircon/get_monitordata
			// bytes 0...3 - prod_site
			// bytes 4...9 - prod_line
			// byte 10 - unknown
			// bytes 11...31 - set to 0xFF, looks like unused part of a ROM
			unknown_v3_cmd(p, response, buf, state->FU15, sizeof(state->FU15));
			break;
		 case '2':
		    // FY25 - reported by BRP072C42 as prod_order= in /aircon/get_monitordata
			unknown_v3_cmd(p, response, buf, state->FU25, sizeof(state->FU25));
			break;
		 case '3':
		    // FY35 - production info. Reported by BRP072C42 in /aircon/get_monitordata
			// bytes 0...1 - inp_site
			// bytes 2 - inp_base
			// bytes 3 - inp_dev
			// bytes 4...11 - inp_mdate
			// bytes 12..23 - inp_serial
			// bytes 24..31 - set to 0xFF, looks like unused part of a ROM
		 ...
		 case '4':
			...
			// bytes 2 - outp_base
			// bytes 3 - outp_dev
			// bytes 4...11 - outp_mdate
			// bytes 12..23 - outp_serial
			// bytes 24..31 - set to 0xFF, looks like unused part of a ROM
